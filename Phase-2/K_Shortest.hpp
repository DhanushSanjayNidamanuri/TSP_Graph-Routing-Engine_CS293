...
public:
    ...
    KShortestPaths_Result(int id, std::vector<std::pair<std::vector<int>,double>> paths) : id(id), paths(paths) {};
};
...
public:
    ...
    std::vector<std::pair<std::vector<int>,double>> KShortest(Graph& graph, int source, int target,unsigned int k, std::string mode);
    std::vector<std::pair<std::vector<int>,double>> KShortest_heuristic(Graph& graph, int source, int target,unsigned int k, int overlap_threshold);
};
